        ...
        if game.CurrentRun.Hero ~= nil then
            if game.CurrentRun.Hero.RallyHealth == nil then
                ...
                if game.CurrentRun.Hero.RallyHealth.RallyDecayRateSeconds == nil then
                    game.CurrentRun.Hero.RallyHealth.RallyDecayRateSeconds = game.HeroData.RallyHealth
                                                                                 .RallyDecayRateSeconds
                end
                ...
                if game.CurrentRun.Hero.RallyHealth.MaxRallyHealthPerHit == nil then
                    game.CurrentRun.Hero.RallyHealth.MaxRallyHealthPerHit = game.HeroData.RallyHealth
                                                                                .MaxRallyHealthPerHit
                end
            end
        end
    end
...
modutil.mod.Path.Wrap("DamageHero", function(base, victim, triggerArgs)
    ...
    if triggerArgs.DamageAmount ~= nil and triggerArgs.DamageAmount > 0 and not triggerArgs.Silent then
        ...
        if game.CurrentRun.Hero.RallyHealth.RallyActive and
            (sourceEffectData == nil or not sourceEffectData.NoRallyStore) and not triggerArgs.PureDamage then
            if game.CurrentRun.Hero.RallyHealth.HitsDrainRallyHealthMultiplier then
                game.CurrentRun.Hero.RallyHealth.Store = game.CurrentRun.Hero.RallyHealth.Store *
                                                             game.CurrentRun.Hero.RallyHealth
                                                                 .HitsDrainRallyHealthMultiplier
            end
            game.CurrentRun.Hero.RallyHealth.Store = game.CurrentRun.Hero.RallyHealth.Store + adjustedDamageAmount *
                                                         game.CurrentRun.Hero.RallyHealth.ConversionPercent
...
function DrainRallyHealth()
    ...
    while game.CurrentRun.Hero.RallyHealth.Store >= 0 do
        if game.CurrentRun.Hero.RallyHealth.State == nil or game.CurrentRun.Hero.RallyHealth.State == "Wait" then
            ...
            drainRate = tickRate * game.CurrentRun.Hero.RallyHealth.Store /
                            game.CurrentRun.Hero.RallyHealth.RallyDecayRateSeconds
...
modutil.mod.Path.Wrap("DashManeuver", function(base, duration)
    if CurrentRun.Hero.RallyHealth.RallyActive and CurrentRun.Hero.RallyHealth.RallyHealOnDash and
        not CurrentRun.Hero.IsDead then
...
-- ??? rewrite this to fit new MetaUpgradeData, idk what this do tbh
...
--     local convperc = newHero.RallyHealth.ConversionPercent

--     convperc = convperc + (GetNumMetaUpgrades("RallyMetaUpgrade") * (MetaUpgradeData.RallyMetaUpgrade.ChangeValue - 1))
--     base(prevRun, args)
-- end)
